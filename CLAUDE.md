...
Если запушить исправление и не задеплоить — сервер продолжит работать со сломанной версией до следующего перезапуска. Бот упадёт в самый неожиданный момент.

**После каждого пуша сразу выполнять деплой** (инструкция ниже).
